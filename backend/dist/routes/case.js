...
router.post("/create", authMiddleware, upload.array("images", 10), async (req, res) => {
    try {
        ...
        if (req.files && Array.isArray(req.files)) {
            uploadedImageUrls = await Promise.all(req.files.map(file => uploadImageToS3(file)));
        }
        // Generate QR Codes and assign images to properties
        const propertiesWithQR = await Promise.all((properties || []).map(async (p, index) => {
...
router.get("/specific/:id", authMiddleware, async (req, res) => {
    try {
        const id = req.params.id; // Type assertion
        if (!id)
...
router.post("/scan/:qrString", authMiddleware, async (req, res) => {
    try {
        const qrString = req.params.qrString; // Type assertion
        const userId = req.userId;
        if (!qrString)
...
router.get("/search", authMiddleware, async (req, res) => {
    const q = req.query.q; // Type assertion
    const userId = req.userId;
